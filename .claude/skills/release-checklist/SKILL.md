...
- [ ] **クライアントのビルドが通るか**
  - `client/` で `yarn build` が成功するか確認する (Create Release Commit ワークフロー内でもビルドされるが、事前に壊れていないか確認しておく)
  - ただし開発サーバーが起動中の場合は port 競合の可能性があるのでユーザーに確認してから実行する
...
- [ ] **サードパーティライブラリのバージョンが最新か**
  ...
  - 特に QSVEncC / NVEncC / VCEEncC / FFmpeg は頻繁に更新されるので、最新版が出ていないかユーザーに確認する
...
- `yarn build` によるクライアントビルド確認 (ユーザーの許可を得てから)
