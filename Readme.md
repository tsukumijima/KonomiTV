...
これは KonomiTV の Windows サービスを一般ユーザーの権限で起動するために利用するもので、入力されたパスワードがそれ以外の用途に利用されることはありません。  
...
その後、インストーラーの実行ファイル (`KonomiTV-Installer.exe`) を Shift + 右クリック → [[別のユーザーとして実行]](https://faq.nec-lavie.jp/qasearch/1007/app/servlet/relatedqa?QID=020525) をクリックして、ログオン中のユーザーとパスワードを指定してから、再度インストーラーを実行してみてください。
