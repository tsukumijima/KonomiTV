...
> **以降のインストール手順は、すべて version 0.5.2 時点のものになります。**  
> **現在開発中の version 0.6.0 ではインストール方法や仕様が大きく変更になっているため、下記に記載の手順でインストールすることはできません。**  
> 動作の保証はありませんが、取り急ぎ [ベータ版のインストーラー](https://github.com/tsukumijima/Storehouse/releases/tag/KonomiTV-Installer-Prerelease) を仮置きしています。開発版 (0.6.0-dev) をお試しになりたい方は、一度インストーラーでのインストールを試してみてください (開発版は 0.5.2 以前との後方互換性がないため、新規インストールがおすすめです)。  
> ドキュメントを整備したいのは山々ですが、片付けなければならない課題が山積していて、まったく手が回っていません…。0.6.0 のリリースまでには整備予定ですので、もうしばらくお待ちいただければ…。
