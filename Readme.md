...
- **「テレビをみる」**: どんなデバイスでもテレビをリアルタイムかつ高画質で視聴でき、ニコニコ実況や Twitter のコメントと一緒に番組を楽しめる
- **「ビデオをみる」**: AbemaTV などの動画配信サービスのような洗練された操作性と視聴体験で、録画番組をストレスなく視聴できる **[🎉NEW!]**
- **「マイリスト」**: あとで見たい番組を登録して視聴予定を管理。見逃しを防ぎ、効率的な番組管理を実現 **[🎉NEW!]**
- **「視聴履歴」**: 視聴した録画番組の履歴を自動で記録し、前回再生を止めた位置からのレジューム再生にも対応 **[🎉NEW!]**
- **「KonomiTV アカウント」**: マイリスト・視聴履歴・コメント設定などのユーザー設定を、複数デバイス間で自動的に同期
...
- **まだ開発中の β 版です。当初よりかなり安定してきましたが、まだ完璧に保証ができる状態ではありません。**
  - **KonomiTV 0.12.0 以降では、構想から4年の歳月を経て録画番組の再生機能が実装されました！🎉🎊**  
    ライブ視聴・録画再生の両方で TVRemotePlus の完全上位互換となっています。TVRemotePlus の開発は終了しているため、移行をお勧めします。
    - ただし、MPEG-TS 形式の録画ファイルのみサポートされています。
...
- **TVRemotePlus の後継という位置づけのソフトですが、それはあくまで精神的なものであり、実際の技術スタックや UI/UX はゼロから設計されています。**
  ...
  - 構想は壮大ですが、構想にあるすべての機能を実装し終えるには、まだまだ時間がかかりそうです。
