...
[Releases](https://github.com/tsukumijima/KonomiTV/releases) にバージョン番号付きで公開されており、インストーラーの `1` / `2` でこのバージョンを導入・更新できます。

**開発版**は、GitHub の `master` ブランチ (開発の本線) に日々取り込まれている最新のコードをそのままインストールできるバージョンです。  
安定版のリリースを待たずに最新の修正や新機能を試せますが、その分動作が不安定になることや、設定の互換性が突然変わることもあります。  
インストーラーの `4` / `5` を選ぶだけで導入・更新でき、専用のパッケージを別途用意する必要はありません。インストーラーのタイトルに表示されるバージョン番号は安定版のものですが、`4` / `5` を選んだ場合は自動的に最新の開発版が取得されます。

迷ったら**安定版 (`1` / `2`) をおすすめします。**  
最新の機能をいち早く試したい場合や、フィードバックに協力したい場合は**開発版 (`4` / `5`)** をお試しください。
