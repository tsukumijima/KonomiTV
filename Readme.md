...
クライアント側のデバッグは `client/` フォルダにて `yarn develop` を実行し、ポート 7001 にてリッスンされるデバッグ用サーバーにて行っています。  
...
`yarn develop` を実行するとコードの変更時に自動的に差分が再ビルドされるため、毎回 `yarn build` を実行し直す必要がありません。  
